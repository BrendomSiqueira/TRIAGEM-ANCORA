          ...
          {user ? (
            <div className="flex items-center gap-4">
              <div className="hidden md:block text-right border-r border-slate-800 pr-6">
                <p className="text-[10px] font-black text-blue-400 uppercase tracking-[0.2em]">{user.displayName || 'USUÁRIO'}</p>
                <p className="text-[9px] text-slate-500 font-bold uppercase">{user.email}</p>
              </div>
              <button 
                onClick={handleLogout}
                className="w-10 h-10 rounded-xl bg-slate-900 border border-slate-800 flex items-center justify-center text-slate-400 hover:text-white hover:bg-red-500/20 hover:border-red-500/30 transition-all"
                title="Sair"
              >
            ...
            <button 
              onClick={handleLogin}
              className="px-6 py-2.5 bg-blue-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-blue-700 transition-all shadow-lg shadow-blue-500/20 flex items-center gap-2"
            >
