...
const App: React.FC = () => {
      ...
      getDoc(clientRef).then(docSnap => {
        if (docSnap.exists()) {
          ...
          if (newProgress !== clientData.progress) {
            ...
            }).catch(err => handleFirestoreError(err, OperationType.UPDATE, `clients/${selectedClientId}`));
          }
        }
      }).catch(err => handleFirestoreError(err, OperationType.GET, `clients/${selectedClientId}`));
    }, (err) => {
      handleFirestoreError(err, OperationType.LIST, `clients/${selectedClientId}/documents`);
    });
  ...
  const handleAddClient = async (newClient: Client) => {
    ...
    try {
      ...
      console.log("Setting doc at path:", path);
  ...
  const handleAddDocument = async (clientId: string, newDoc: ClientDocument) => {
    ...
    const path = `clients/${clientId}/documents`;
    try {
  ...
  const handleRemoveDocument = async (clientId: string, docId: string) => {
    const docPath = `clients/${clientId}/documents/${docId}`;
    try {
